#include "t2kSplineFactory.h"

using namespace std;

const double t2kSplineFactory::sigvals[13] = {-3., -2.5, -2., -1.5, -1., -0.5, 0., 0.5, 1., 1.5, 2., 2.5, 3.};
const double t2kSplineFactory::maqevals[21] = {-7., -6.5, -6., -5.5, -5., -4.5, -4., -3.5, -3., -2.5, -2., -1.5, -1., -0.5, 0., 0.5, 1., 1.5, 2., 2.5, 3.};
const double t2kSplineFactory::binarys[2] = {0., 1.};

////////////////////////////////////////////////
//run spline factory using parameters from file
void t2kSplineFactory::runSplineFactory(){
...
void t2kSplineFactory::resetModHistos(){
  if (!separateNeutMode) {
...
void t2kSplineFactory::setMCTree(TChain* tr){
  mcTree = tr;
  mcEvt = new t2kfqReader(mcTree);
  mcEvt->FillMap();
  return;
}

void t2kSplineFactory::debugtest(){
  //create new factory
  t2kSplineFactory* sfactory = new t2kSplineFactory(3,3,7,1,1,"debugtest");
  //set histogram manager and create template histograms
  sfactory->makeManagerFromFile("factoryOut_factorytest.root");
  return;
};

void t2kSplineFactory::fillLeaves(int isamp,int ibin,int icomp,int iatt,int isyst, int imode){
...
void t2kSplineFactory::buildTheSplines(){
...
void t2kSplineFactory::incrementSystPars(double nsig, int isyst){
  setupSystPars();
...
void t2kSplineFactory::incrementSystPars(double nsig){
...
void t2kSplineFactory::setupSystPars(){
...
void t2kSplineFactory::setupHistos(){
...
void t2kSplineFactory::makeManagerFromFile(const char* fname){
  if (!separateNeutMode) {
...
int t2kSplineFactory::getBest2RFitID(){
...
void t2kSplineFactory::fillAttributes(){
...
void t2kSplineFactory::fillHistograms(int ipt, int isyst){
...
double t2kSplineFactory::getEvtWeight(t2kfqReader* mcEvt,int ipar,double value){
//  double ww = 1.;
...
double t2kSplineFactory::getEvtWeight(int ipar){
//  double ww = 1.;
  double ww = mcEvt->evtweight;
  int absmode = TMath::Abs(mcEvt->mode);
...
t2kSplineFactory::t2kSplineFactory(const char*  parfile, bool separateneutmode)
  : separateNeutMode(separateneutmode)
{
  parFileName = parfile;
  cout<<"t2kSplineFactory: Parameter File: "<<parfile<<endl;
...
void t2kSplineFactory::setAtmFitPars(atmFitPars *a)
{
  fitPars = a;
}

t2kSplineFactory::t2kSplineFactory(int isamp, int ibin, int icomp, int iatt, int isyst, const char* name, int nmode, bool separateneutmode)
  : nMode(nmode)
  , separateNeutMode(separateneutmode)
{
  nameTag = name;
  ...
  nAtt  = iatt;
  nSyst = isyst;
  foutName = "t2kSplineFactoryOutput.root";
}
