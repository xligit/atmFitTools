void test_covBANFF()
{
  ...
  covBANFF *cov_postfit = new covBANFF("postfit_cov", "/home/xiaoyue/atmFitTools/rootfiles/postfit_data_1p1h_biascorrection_20160310.root", 1, true);
  //cov_postfit->PrintPars();
  cov_postfit->PrintNominal();
  cov_postfit->proposeStep();
  cov_postfit->proposeStep();
  cov_postfit->acceptStep();
  cov_postfit->PrintPars();
